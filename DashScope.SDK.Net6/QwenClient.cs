...
namespace DashScope.SDK.Net6
{
    public  class QwenClient
    {
        ...
        /// <returns></returns>
        public async Task<ApiOutputResult> QwenCallAsync(ChatRequestParam requestParam, Endpoint endpoint, bool isMultipleConversation = false, bool isAsycn = false)
        {
            ...
            var httprequest = requestParam.GetHttpRequestMessage(_apiKey, HttpMethod.Post, Defaults.Endpoint(endpoint), isAsycn);
            ...
            if (isMultipleConversation)
            {
                requestParam.BuildHistory(result.Output!.Text, new Message() { Role = "user", Content = requestParam.Prompt });
            }
            return result;
        }
